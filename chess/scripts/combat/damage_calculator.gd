...
static func apply(attacker: BattleUnit, target: BattleUnit, damage_multiplier: int = 1) -> Dictionary:
	if attacker == null or target == null or attacker.attack < 0 or damage_multiplier < 1:
		push_error("[DamageCalculator.apply] invalid combatant or attack value")
		return {}
	var damage_value := attacker.attack * damage_multiplier
	var result := {
	...
	if attacker.true_damage:
		target.current_hp = maxi(target.current_hp - damage_value, 0)
	elif target.current_shield > 0:
		target.current_shield = maxi(target.current_shield - damage_value, 0)
